...
@pytest.mark.asyncio
async def test_process_media_success(tmp_path):
    ...
    with patch("asyncio.create_subprocess_exec", return_value=mock_process):
        ...
        await ffmpeg.process_media(input_path, output_path, "job-1")

@pytest.mark.asyncio
async def test_process_audio_success(tmp_path):
    """Verifies that an audio file can also be processed."""
    input_path = tmp_path / "input.mp3"
    output_path = tmp_path / "output_optimized.mp3"
    input_path.touch()
    
    mock_process = MagicMock()
    mock_process.returncode = 0
    mock_process.stdout.readline = AsyncMock(side_effect=[b"out_time_ms=500000\n", b""])
    mock_process.stderr.read = AsyncMock(side_effect=[b"ffmpeg logs", b""])
    mock_process.wait = AsyncMock(return_value=0)
    
    with patch("asyncio.create_subprocess_exec", return_value=mock_process):
        output_path.write_text("optimized audio data")
        await ffmpeg.process_media(input_path, output_path, "job-audio")
